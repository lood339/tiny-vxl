...
TEST(vgl_area, simple1)
{
    double cont1[] = { 0,0,  5,0,  5,3,  0,3 };
    vgl_polygon<double> poly1(cont1, 4);
    ASSERT_NEAR(vgl_area_signed(poly1), 15, 1e-6)<<"rectangle (ccw) signed\n";
    ASSERT_NEAR(vgl_area(poly1), 15, 1e-6)<<"rectangle (ccw) unsigned\n";
    vgl_point_2d<double> c = vgl_centroid(poly1);
    ASSERT_NEAR(c.x(), 2.5, 1e-6)<<"rectangle (ccw) centroid x\n";
    ASSERT_NEAR(c.y(), 1.5, 1e-6)<<"rectangle (ccw) centroid y\n";
}

TEST(vgl_area, simple2)
{
    float cont1[] = { 0,0,  0,5,  4,5,  4,0 };
    vgl_polygon<float> poly1(cont1, 4);
    ASSERT_NEAR(vgl_area_signed(poly1), -20, 1e-6)<<"rectangle (ccw) signed\n";
    ASSERT_NEAR(vgl_area(poly1), 20, 1e-6)<<"rectangle (ccw) unsigned\n";
    vgl_point_2d<float> c = vgl_centroid(poly1);
    ASSERT_NEAR(c.x(), 2.0, 1e-6)<<"rectangle (ccw) centroid x\n";
    ASSERT_NEAR(c.y(), 2.5, 1e-6)<<"rectangle (ccw) centroid y\n";
}
