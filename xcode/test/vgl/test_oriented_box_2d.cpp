...
//#include <vgl/vgl_fit_oriented_box_2d.h>  @todo move to algo test

#include <gtest/gtest.h>

TEST(oriented_box_2d, simple)
{
    std::cout << "***************************\n"
    ...
    EXPECT_EQ(trans_corns1[2].x()+trans_corns1[2].y(), 475.0)<<"translate obox\n";
    
    vgl_point_2d<double> cent = tbox1.centroid();
    EXPECT_EQ(cent.x()+cent.y(), 400.0)<<"centroid\n";
    ...
    ASSERT_NEAR(area, 5000.0, 0.000001)<<"bounding box\n";
    ...
    EXPECT_EQ(width == 200.0f && height == 150.0f, true)<<"box size\n";
    EXPECT_EQ(corns[2]==vgl_point_2d<float>(200.0f, 175.0f), true)<<"corners \n";
    EXPECT_EQ(uv == vgl_point_2d<float>(-75.0f/200.0f, -0.5f), true)<<"uv \n";
    
    // test contains
    vgl_point_2d<float> pin(199.0, 140.0);
    vgl_point_2d<float> pout(175.0, 180.0);
    bool good = ob0.contains(pin) && !ob0.contains(pout);
    EXPECT_EQ(good , true)<<"contains \n";
    vgl_point_2d<float> p0(0.0f, 25.0f), p1(200.0f, 25.0f), p2(0.0f, 175.0);
    vgl_oriented_box_2d<float> ob1a(p0, p1, p2);
    EXPECT_EQ(ob1a == ob0, true)<<"construct from 3 pts\n";
    ...
    EXPECT_EQ(ob2a==ob3, true)<<"test rotation constructors\n";
    ...
    ASSERT_NEAR(ang, 0.523599f, 0.001)<<"orientation angle \n";
}

/*
 // @todo move to vgl_fit_oriented_box_2d test
