...
TEST(vnl_quaternion, random_round_trip)
{
  ...
  EXPECT_EQ(errcount, 0)<<"1000*Random euler -> quaternion -> euler consistent\n";
  std::cout << "Average squared error: " << avg_sqr_error << std::endl;
}

TEST(vnl_quaternion, random_euler_near_zero)
{
  vnl_random rng(13241ul);
  unsigned errcount = 0;
  double avg_sqr_error = 0.0;
  ...
  EXPECT_EQ(errcount, 0)<<"1000*Random small euler -> small quaternion angle\n";
  std::cout << "Average squared error: " << avg_sqr_error << std::endl;
}

TEST(vnl_quaternion, random_quat_near_zero)
{
  vnl_random rng(13241ul);
  unsigned errcount = 0;
  for (unsigned i = 0; i < 1000; ++i)
  ...
  EXPECT_EQ(errcount, 0)<<"1000*Random small quat -> small euler values\n";
}
...
TEST(vnl_quaternion, rotations)
{
  vnl_vector_fixed<double, 3> p1(2, 2, 2), p2(1, 0, 0), p3(0, 1, 0);
  vnl_vector_fixed<double, 3> e0(0, 0, 0);
  vnl_quaternion<double> q0(0, 0, 0, 0);
  ASSERT_NEAR(vnl_vector_ssd(q0.rotate(p1), p1), 0.0, 1e-8)<<"rotate p1 using q0\n";
  ASSERT_NEAR(vnl_vector_ssd(q0.rotate(p2), p2), 0.0, 1e-8)<<"rotate p2 using q0\n";
  vnl_quaternion<double> q0_b(0, 0, 0, 1);
  ASSERT_NEAR(vnl_vector_ssd(q0_b.rotate(p1), p1), 0.0, 1e-8)<<"rotate p1 using q0_b\n";
  ASSERT_NEAR(vnl_vector_ssd(q0_b.rotate(p2), p2), 0.0, 1e-8)<<"rotate p2 using q0_b\n";
  ASSERT_NEAR(vnl_vector_ssd(q0_b.rotation_euler_angles(), e0), 0.0, 1e-8)<<"q0_b -> Euler angles\n";
  std::cout << "q0_b -> Euler angles: " << q0_b.rotation_euler_angles() << std::endl;
  vnl_quaternion<double> q0_c(0, 0, 0, -4);
  ASSERT_NEAR(vnl_vector_ssd(q0_c.rotate(p1), p1), 0.0, 1e-8)<<"rotate p1 using q0_c\n";
  ASSERT_NEAR(vnl_vector_ssd(q0_c.rotate(p2), p2), 0.0, 1e-8)<<"rotate p2 using q0_c\n";
  vnl_quaternion<double> q0_d(0, 0, 0);
  ASSERT_NEAR(vnl_vector_ssd(q0_d.rotate(p1), p1), 0.0, 1e-8)<<"rotate p1 using q0_d\n";
  ASSERT_NEAR(vnl_vector_ssd(q0_d.rotate(p2), p2), 0.0, 1e-8)<<"rotate p2 using q0_d\n";
  ...
  ASSERT_NEAR(vnl_vector_ssd(q1.rotate(p1), p1), 0.0, 1e-8)<<"rotate p1 using q1\n";
  ASSERT_NEAR(vnl_vector_ssd(q1.rotate(p2), p3), 0.0, 1e-8)<<"rotate p2 using q1\n";
  vnl_vector_fixed<double, 3> e1_b = q1.rotation_euler_angles();
  ASSERT_NEAR(vnl_vector_ssd(e1_b, e1), 0.0, 1e-8)<<"q1 -> Euler angles\n";
  vnl_quaternion<double> q1_c = -q1;
  vnl_vector_fixed<double, 3> e1_c = q1_c.rotation_euler_angles();
  ASSERT_NEAR(vnl_vector_ssd(e1_c, e1), 0.0, 1e-8)<<"-q1 -> Euler angles\n";

  std::cout << "q1 -> Euler angles: " << e1 << std::endl;
  vnl_quaternion<double> q1_b(e1(0), e1(1), e1(2));
  std::cout << "q1 -> Euler angles: " << q1_b << std::endl;
  ASSERT_NEAR(vnl_vector_ssd(q1_b, q1), 0.0, 1e-8)<<"Euler angles -> q1\n";
