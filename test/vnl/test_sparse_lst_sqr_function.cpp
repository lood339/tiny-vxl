...
TEST(vnl_sparse_lst_sqr_function, test_sparse_lst_sqr_function)
{
  ...
  EXPECT_EQ(my_func.number_of_a(), 3)<<"number_of_a\n";
  EXPECT_EQ(my_func.number_of_b(), 4)<<"number_of_b\n";
  EXPECT_EQ(my_func.number_of_e(), 9)<<"number_of_e\n";
  ...
  EXPECT_EQ(num_valid, true)<<"number_of_params_a\n";
  EXPECT_EQ(index_valid, true)<<"index_a\n";
  ...
  EXPECT_EQ(num_valid, true)<<"number_of_params_b\n";
  EXPECT_EQ(index_valid, true)<<"index_b\n";

  EXPECT_EQ(2, my_func.number_of_params_c())<<"number_of_params_c\n";
  ...
  EXPECT_EQ(num_valid, true)<<"number_of_residuals\n";
  EXPECT_EQ(index_valid, true)<<"index_e\n";
  ...
  std::cout << "Aij =\n" << Aij << std::endl << "fd Aij =\n" << fd_Aij << std::endl;
  EXPECT_EQ((Aij - fd_Aij).absolute_value_max() < 0.001, true)<<"finite difference Aij\n";
  std::cout << "Bij =\n" << Bij << std::endl << "fd Bij =\n" << fd_Bij << std::endl;
  EXPECT_EQ((Bij - fd_Bij).absolute_value_max() < 0.001, true)<<"finite difference Bij\n";
  std::cout << "Cij =\n" << Cij << std::endl << "fd Cij =\n" << fd_Cij << std::endl;
  EXPECT_EQ((Cij - fd_Cij).absolute_value_max() < 0.001, true)<<"finite difference Cij\n";
  ...
  for (unsigned int i = 0; i < my_func.number_of_a(); ++i)
  {
    for (unsigned int j = 0; j < my_func.number_of_b(); ++j)
    {
      ...
      if (k < 0)
        continue;
      ASSERT_NEAR("valid weights", weights[k], double((i + 1) * (j + 1)) / w_norm, 1e-10);
      EXPECT_EQ("computed weight wA", wA[k], A[k] * weights[k]);
      EXPECT_EQ("computed weight wB", wB[k], B[k] * weights[k]);
      EXPECT_EQ("computed weight wC", wC[k], C[k] * weights[k]);
      ASSERT_NEAR("computed weight wf", wf[2 * k], f[2 * k] * weights[k], 1e-10);
      ASSERT_NEAR("computed weight wf", wf[2 * k + 1], f[2 * k + 1] * weights[k], 1e-10);
    }
  }
  EXPECT_EQ("has weights", my_func.has_weights(), true);
     */
}
