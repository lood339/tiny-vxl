...
TEST(vpgl_generic_camera, simple_test)
{
    ...
    ASSERT_NEAR(er, 0.0, 0.0001)<<"interpolated ray\n";
    ...
    ASSERT_NEAR(er, 0.0, 0.0001)<<"project\n";
}

TEST(vpgl_generic_camera, proj_test)
{
    unsigned ni = 640;
    unsigned nj = 480;
    ...
    for (std::vector<vgl_point_3d<double>>::const_iterator pit = test_pts.begin(); pit != test_pts.end(); ++pit)
    {
        ...
        ASSERT_NEAR(u, p2d.x(), 1e-3)<<testname.str().c_str()<<std::endl;
        testname.str("");
        testname << "Projection of test point " << *pit << ": v" << std::endl;
        ASSERT_NEAR(v, p2d.y(), 1e-3)<<testname.str()<<std::endl;
    }
    ...
    for (std::vector<vgl_point_2d<double>>::const_iterator pit = test_pts1.begin(); pit != test_pts1.end(); ++pit)
    {
        ...
        ASSERT_NEAR(r.origin().x(), ray.origin().x(), 1e-3)<<testname.str()<<std::endl;
        testname.str("");
        testname << "Ray origin for test point " << *pit << ": y" << std::endl;
        ASSERT_NEAR(r.origin().y(), ray.origin().y(), 1e-3)<<testname.str()<<std::endl;;
        testname.str("");
        testname << "Ray origin for test point " << *pit << ": z" << std::endl;
        ASSERT_NEAR(r.origin().z(), ray.origin().z(), 1e-3)<<testname.str()<<std::endl;
        testname.str("");
        testname << "Ray direction for test point " << *pit << ": dx" << std::endl;
        ASSERT_NEAR(r.direction().x(), ray.direction().x(), 1e-3)<<testname.str()<<std::endl;;
        testname.str("");
        testname << "Ray direction for test point " << *pit << ": dy" << std::endl;
        ASSERT_NEAR(r.direction().y(), ray.direction().y(), 1e-3)<<testname.str()<<std::endl;;
        testname.str("");
        testname << "Ray direction for test point " << *pit << ": dz" << std::endl;
        ASSERT_NEAR(r.direction().z(), ray.direction().z(), 1e-3)<<testname.str()<<std::endl;;
    }
}
