...
TEST(vpgl_perspective_camera, perspective_camera)
{
  ...
  ASSERT_NEAR(hp_2d.x() / hp_2d.w(), 512, 1e-06)<<"test projection to principal point\n";

  vgl_homg_point_3d<double> hpa_3d(10, 20, 10);
  hpa_2d = P(hpa_3d);
  std::cout << "p3da " << hpa_3d << " p2da " << hpa_2d << '\n';
  ASSERT_NEAR(hpa_2d.x() / hpa_2d.w(), 1512, 1e-06)<<"test x projection of arbitrary point\n";

  ASSERT_NEAR(hpa_2d.y() / hpa_2d.w(), 2384, 1e-06)<<"test y projection of arbitrary point\n";
  ...
  // test look_at
  {
    ...
    EXPECT_EQ(infront, true)<<"look_at - in front of camera\n";
    ASSERT_NEAR((tgt - K.principal_point()).length(), 0.0, 1e-8)<<"look_at - target projects to prin. pt.\n";
    // a point above the target should project above in the image (-Y is "up" in the image)
    EXPECT_EQ((tgt.x() - abv.x()) < 1e-8 && tgt.y() > abv.y(), true)<<"look_at - correct direction is up\n";
  }
    
  vgl_homg_point_3d<double> X(10., 0, 0);
  vgl_point_2d<double> hur = P_rot.project(X);
  std::cout << X << '\n' << hur << '\n';
  ASSERT_NEAR(hur.x(), 5340.43, 0.01)<<"test postmultipy with pure rotation\n";
  ...
  std::cout << X << '\n' << hurt << '\n';
  ASSERT_NEAR(hurt.x(), 7843.59, 0.01)<<"test postmultipy with general Euclidean\n";
  ...
  // test align up/down
  {
    ...
    ASSERT_NEAR((P2.get_matrix() - P1.get_matrix()).frobenius_norm(), 0.0, .01)<<"testing align up/down:\n";
  }
  ...
  //std::ifstream is(cam_path.c_str());
    vpgl_perspective_camera<double> Pin = Po;
  //is >> Pin;
  ...
  // test constructors and methods using translation
    ...
    ASSERT_NEAR(dif, 0.0, 1.0e-04)<<"Perspective frustum\n";
    ...
    EXPECT_EQ(cont && not_cont, true)<<"point_in_actual_frustum\n";
     */
}
