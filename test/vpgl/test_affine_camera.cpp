...
TEST(vpgl_affine_camera, affine_camera)
{
  ...
  ASSERT_NEAR(u00 + v00, 100, 1e-5)<<"test center projection\n";
  C.project(x1, y1, z1, u1, v1);
  C.project(x2, y2, z2, u2, v2);
  C.project(x3, y3, z3, u3, v3);
  ASSERT_NEAR(v1 + u2 + u3, 49.591751709536 + 50 - sq2 + 50 + sq2, 1e-9)<<"test du, dv\n";
  ...
  ASSERT_NEAR(len, 0.0, 1e-8)<<"Camera center\n";
  ...
  ASSERT_NEAR((row_ray_dir - test_dir).length(), 0.0, 1e-5)<<"row ray dir\n";
  ...
  ASSERT_NEAR(ray_dist, 0.0, 0.05)<<"row_ray_dist\n";
  ...
  ASSERT_NEAR(pdist, 0.0, 0.0001)<<"postmultiply translation\n";
}
