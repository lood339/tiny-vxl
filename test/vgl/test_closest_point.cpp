...
TEST(closest_point, HomgLine2D)
{
    ...
    EXPECT_EQ(p, q)<<"2D coincident test\n";
    EXPECT_EQ(vgl_closest_point(q,l), q)<<"2D coincident test\n";
    ASSERT_NEAR(vgl_distance(l,q), 0.0, 1e-8);
    ASSERT_NEAR(vgl_distance(q,l), 0.0, 1e-8);
    q.set(5,1); p = vgl_closest_point(l,q);
    EXPECT_EQ(p, q)<<"2D coincident test\n";

    // test for non-coincident
    q.set(0,2); p.set(0,1);
    EXPECT_EQ(vgl_closest_point(l,q), p)<<"2D non-coincident test\n";
    ASSERT_NEAR(vgl_distance(l,q), 1.0, 1e-8)<<"Distance test\n";
    q.set(5,2); p.set(5,1);
    EXPECT_EQ(vgl_closest_point(q,l), p)<<"2D non-coincident test\n";
    ASSERT_NEAR(vgl_distance(q,l), 1.0, 1e-8)<<"Distance test\n";
}


// Test for closest points on two 3D lines, by Brendan McCane

TEST(closest_point, HomgLine3D)
{
    ...
    EXPECT_EQ(pts.first, pts.second)<<"Parallel test\n";
    EXPECT_EQ(pts.first,p2)<<"Parallel test\n";
    
    ASSERT_NEAR(vgl_distance(l1,l2), 5.0, 1e-8)<<"Parallel distance test\n";
    ASSERT_NEAR(vgl_distance(l1,p1), 5.0, 1e-8)<<"Parallel distance test\n";
    p1.set(0,1,0);
    ASSERT_NEAR(vgl_distance(p1,l2), 5.0, 1e-8)<<"Parallel distance test\n";
    ...
    std::cout << "Intersect test failed, points should be " << p2 << '\n'
             << "points are: " << pts.first << ' ' << pts.second
             << std::endl;
    EXPECT_EQ(pts.first==pts.second && pts.first==p2, true)<<"Intersect test\n";
    ASSERT_NEAR(vgl_distance(l1,l2), 0.0, 1e-8)<<"Intersect distance test\n";
    ...
    EXPECT_EQ(pts.first, vgl_homg_point_3d<double>(2,2,0,3))<<"Skew lines test\n";
    EXPECT_EQ(pts.second,vgl_homg_point_3d<double>(3,1,1,3))<<"Skew lines test\n";
    ASSERT_NEAR(vgl_distance(pts.first,pts.second),1/std::sqrt(3.0),1e-8)<<"Skew lines distance test\n";
    ASSERT_NEAR(vgl_distance(l1,pts.second),1/std::sqrt(3.0),1e-8)<<"Skew lines distance test\n";
    ASSERT_NEAR(vgl_distance(pts.first,l2),1/std::sqrt(3.0),1e-8)<<"Skew lines distance test\n";
}
