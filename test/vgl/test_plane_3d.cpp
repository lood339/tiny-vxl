...
TEST(plane_3d, constructor)
{
  ...
  ASSERT_NEAR(dist ,0.0, 1e-5)<<"Constructors and Normal\n";
  ...
  ASSERT_NEAR(dist ,0.0, 1e-5)<<"Coincident ray constructor\n";
  ...
  ASSERT_NEAR(dist ,0.0, 1e-5)<<"Parallel ray constructor\n";
}



TEST(plane_3d, operations)
{
  ...
  good = good && (er < 1e-5);
    EXPECT_EQ(good, true)<<"Plane Coordinate System: world -> plane\n";
  ...
  ASSERT_NEAR(er,0.0,1e-5)<<"Plane Coordinate System: plane -> world\n";
  ...
  good = plane_real.plane_coords(pt3d_on, pt2d, 1e-5);
    EXPECT_EQ(good, true)<<"Realistic plane test: world -> plane\n";
  vgl_point_3d<double> prec_3d = plane_real.world_coords(pt2d);
  er = (prec_3d-pt3d_on).length();
  ASSERT_NEAR(er, 0.0, 1e-5)<<"Realistic plane test\n";
  ...
  ss >> prec;
  EXPECT_EQ(pstr == prec, true)<<"plane stream ops\n";
