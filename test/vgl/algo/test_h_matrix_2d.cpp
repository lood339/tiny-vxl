...
TEST(h_matrix_2d, identity_transform)
{
    ...
    std::cout << "Id\n" << Id << '\n' << "p =" << p << " , Id(p) = pp =" << pp << '\n';
    ASSERT_NEAR(length(p - pp), 0.0, 1e-06);
}


TEST(h_matrix_2d, perspective_transform)
{
...
TEST(vgl_h_matrix_2d, transform_types)
{
    ...
    h.set_affine(M);
    pp = h(p);
    ASSERT_NEAR(length(pp - tasp), 0.0, 1e-05)<<"affine\n";
}

TEST(vgl_h_matrix_2d, projective_basis)
{
    ...
    EXPECT_EQ(Basis(p0), vgl_homg_point_2d<double>(1, 0, 0))<<"transformed 1st point\n";
    EXPECT_EQ(Basis(p1), vgl_homg_point_2d<double>(0, 1, 0))<<"transformed 2nd point\n";
    ASSERT_NEAR(length(Basis(p2) - p0), 0.0, 1e-06)<<"transformed 3rd point\n";
    ASSERT_NEAR(length(Basis(p3) - p3), 0.0, 1e-06)<<"transformed 4th point\n";
    ...
    EXPECT_EQ(collinear_basis.projective_basis(collinear_points), false)<<"collinear degeneracy\n";
}


TEST(vgl_h_matrix_2d, projective_basis_from_lines)
{
  ...
  ASSERT_NEAR(lu.b() * lu.b() + lu.c() * lu.c(), 0.0, 1e-12)<<"transformed 1st line\n";
  lu = Basis(l1);
  ASSERT_NEAR(lu.a() * lu.a() + lu.c() * lu.c(), 0.0, 1e-12)<<"transformed 2nd line\n";
  lu = Basis(l2);
  ASSERT_NEAR(lu.a() * lu.a() + lu.b() * lu.b(), 0.0, 1e-12)<<"transformed 3rd line\n";
  lu = Basis(l3);
  ASSERT_NEAR(
    (lu.a() - lu.c()) * (lu.a() - lu.c()) + (lu.b() - lu.c()) * (lu.b() - lu.c()), 0.0, 1e-12)<<"transformed 4th line\n";
