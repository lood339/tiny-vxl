...
test_conversions(const vgl_rotation_3d<double> & rot)
{
  ...
  ASSERT_NEAR(diff1, 0.0, epsilon)<<"Matrix conversion\n";
  if (diff1 > epsilon)
    std::cout << "Matrix:\n" << R << std::endl;
  ...
  ASSERT_NEAR(diff2, 0.0, epsilon)<<"3D Homography conversion\n";
  if (diff2 > epsilon)
  ...
  ASSERT_NEAR(diff3, 0.0, epsilon)<<"Rodrigues conversion\n";
  if (diff3 > epsilon)
  ...
  ASSERT_NEAR(diff4, 0.0, epsilon)<<"Euler conversion\n";
  if (diff4 > epsilon)
  ...
  ASSERT_NEAR(err, 0.0, epsilon)<<"Flip vector\n";
}
...
test_inverse(const vgl_rotation_3d<double> & rot)
{
  ...
  ASSERT_NEAR(diff, 0.0, epsilon)<<"Inverse rotation\n";
}
...
test_transpose(const vgl_rotation_3d<double> & rot)
{
  ...
  double diff = (I.as_quaternion() - vgl_rotation_3d<double>().as_quaternion()).magnitude();
  ASSERT_NEAR(diff, 0.0, epsilon)<<"transpose or conjugate rotation\n";
}
...
test_application(const vgl_rotation_3d<double> & rot)
{
  ...
  ASSERT_NEAR(vgl_distance(hpt, hpl), vgl_distance(r_hpt, r_hpl), epsilon)<<"Rotated point-plane dist (homg)\n";

  double diff = (r_p - vnl_double_3(r_hpt.x() / r_hpt.w(), r_hpt.y() / r_hpt.w(), r_hpt.z() / r_hpt.w())).magnitude();
  ASSERT_NEAR(diff, 0.0, epsilon)<<"Rotated point\n";
  ...
  ASSERT_NEAR(vgl_distance(pt, pl), vgl_distance(r_pt, r_pl), epsilon)<<"Rotated point-plane dist\n";
  ...
  ASSERT_NEAR((r_v - r_v2).length(), 0.0, epsilon)<<"Rotated vector\n";
...
TEST(vgl_rotation_3d, rotation_3d)
{
    std::cout << "*************************\n"
    ...
    ASSERT_NEAR(errorp, 0.0, epsilon)<<"constructor from 2 vectors: rotate 45d around Y axis\n";
    double errorm = (b * vnl_math::sqrt2 - am_to_b).squared_magnitude();
    ASSERT_NEAR(errorm, 0.0, epsilon)<<"constructor from 2 vectors: rotate 225d around Y axis\n";
    double error1 = (b - a1_to_b).squared_magnitude();
    ASSERT_NEAR(error1, 0.0, epsilon)<<"constructor from 2 vectors: from arbitrary point\n";
    double error2 = (b - a2_to_b).squared_magnitude();
    ASSERT_NEAR(error2, 0.0, epsilon)<<"constructor from 2 vectors: 180 degree rotation\n";
    ...
    ASSERT_NEAR(errorf, 0.0f, 1e-6f)<<"constructor from two vgl vectors\n";
