...
TEST(vgl_clip, union)
{
    ...
    
    {
        ...
        EXPECT_EQ(
             result.num_sheets() == 1 && result.num_vertices() == 4,  true)<<"union with null polygon 1 \n";
        
        vgl_polygon<double> result2 = vgl_clip( poly2, poly1, vgl_clip_type_union );
        EXPECT_EQ(
             result2.num_sheets() == 1 && result2.num_vertices() == 4, true)<<"union with null polygon 2\n";
    }
    
    {
        ...
        vgl_polygon<double> result = vgl_clip( poly1, poly2, vgl_clip_type_union );
        EXPECT_EQ(
             result.num_sheets() == 2 && result.num_vertices() == 7, true)<<"disjoint union\n";
    }
    ...
    #ifdef BUILD_NONCOMMERCIAL
    {
        ...
        EXPECT_EQ(
             ...
             is_vertex(result, 5.0,1.0) && is_vertex(result, 4.0,6.0), true)<<"overlapping union\n";
    }
