...
template <class T>
class VNL_EXPORT vnl_vector_ref : public  Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1, Eigen::ColMajor>>
{
 ...
 public:
  ...
  //        can preserve memory access.
    ...
    Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1, Eigen::ColMajor>>(space, n, 1)
    {}
...
: vnl_vector<T>(space, n, false)
{
}

template <typename T>
vnl_vector_ref<T>::vnl_vector_ref(const vnl_vector_ref<T> & v)
: vnl_vector<T>(const_cast<T *>(v.data_block()), v.size(), false)
{}
