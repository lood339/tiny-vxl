...
// copy from .cpp file
constexpr double maxint_as_double = static_cast<double>(vnl_numeric_traits<vnl_rational::int_type>::maxval);

template <typename FloatingType>
inline void
makeNumDen(FloatingType d, vnl_rational::int_type & num_, vnl_rational::int_type & den_)
{
    bool sign = d < 0;
    if (sign)
        d = -d;
    ...
    vnl_rational::int_type den = 0L, num = 1L, prev_den = 1L, prev_num = 0L;
    
    while (d * num < 1e9 && d * den < 1e9)
    {
        vnl_rational::int_type a = static_cast<vnl_rational::int_type>(d); // integral part of d
        d -= a;                                // certainly >= 0
        vnl_rational::int_type temp = num;
        num = a * num + prev_num;
        prev_num = temp;
        temp = den;
        den = a * den + prev_den;
